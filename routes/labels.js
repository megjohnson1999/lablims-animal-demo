...
router.post('/generate', auth, async (req, res) => {
  try {
    ...
    // Create header row
    let labelContent = 'LABELNAME\tLABEL\tEXTID\tSPECIMENTYPE\tDate\tLabelQuantity\tEND\n';
    
    // Add a row for each specimen
    specimens.rows.forEach(specimen => {
      const dateCollected = specimen.date_collected 
        ? new Date(specimen.date_collected).toISOString().split('T')[0]
        : '';
        
      labelContent += `${labelTemplate}\t${specimen.tube_id || specimen.id}\t${specimen.patient_external_id || ''}\t${specimen.specimen_type || ''}\t${dateCollected}\t1\n`;
    });
