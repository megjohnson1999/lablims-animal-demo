...
router.get('/', auth, async (req, res) => {
  try {
    ...
    // Get paginated results with legacy ID
    let dataQuery = `
      SELECT c.*
      FROM collaborators c
      WHERE c.collaborator_number != 0
    ...
    if (search) {
      ...
      dataQuery += ` AND ${searchResult.whereClause} ORDER BY c.collaborator_number, c.pi_name
      LIMIT $${nextParamIndex} OFFSET $${nextParamIndex + 1}`;
      dataParams = [...searchResult.parameters, limit, offset];
    } else {
      dataQuery += ` ORDER BY c.collaborator_number, c.pi_name
      LIMIT $1 OFFSET $2`;
...
// @desc    Get all projects for a collaborator
// @access  Private
router.get('/:id/projects', auth, async (req, res) => {
  try {
    const result = await db.query(
      'SELECT * FROM projects WHERE collaborator_id = $1 ORDER BY date_received DESC',
      [req.params.id]
    );
