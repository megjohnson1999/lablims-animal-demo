...
router.post(
  ...
  async (req, res) => {
    ...
    try {
      ...
      // Validate analysis_status if provided
      const validAnalysisStatuses = ['pending', 'in_progress', 'completed', 'failed'];
      if (analysis_status && !validAnalysisStatuses.includes(analysis_status)) {
        await client.query('ROLLBACK');
        return res.status(400).json({ msg: 'Invalid analysis status. Must be: pending, in_progress, completed, or failed' });
      }
      ...
      const result = await client.query(
        ...
        (project_id, patient_id, tube_id, extracted, initial_quantity, 
         ...
         used_up, specimen_site, run_number, comments, sequencing_run_id, fastq_location,
         analysis_status, results_location, sequencing_notes, specimen_number, metadata) 
        ...
        [
          ...
          nucleated_cells, cell_numbers, percentage_segs, csf_protein, csf_gluc,
          used_up, specimen_site, run_number, comments, sequencing_run_id, fastq_location,
          analysis_status, results_location, sequencing_notes, specimen_number, JSON.stringify(metadata)
        ]
      );
    ...
    try {
      ...
      const result = await client.query(
        ...
        SET project_id = $1, patient_id = $2, tube_id = $3, extracted = $4, initial_quantity = $5,
            ...
            sequencing_run_id = $24, fastq_location = $25, analysis_status = $26,
            results_location = $27, sequencing_notes = $28
        WHERE id = $29
        RETURNING *`,
        [
          project_id, patient_id, tube_id, extracted, initial_quantity,
          ...
          sequencing_run_id, fastq_location, analysis_status,
          results_location, sequencing_notes,
          req.params.id
        ]
      );
