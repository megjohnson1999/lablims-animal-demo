...
BEGIN
    IF NOT EXISTS (
        ...
        WHERE table_name = 'housing' AND column_name = 'status'
    ) THEN
        ALTER TABLE housing ADD COLUMN status VARCHAR(20) DEFAULT 'active';
        RAISE NOTICE 'Added status column to housing table';
    ELSE
        RAISE NOTICE 'Status column already exists in housing table';
    END IF;
END $$;
...
DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'biological_samples') THEN
        ...
        -- Create indexes
        CREATE INDEX idx_biological_samples_number ON biological_samples(sample_number);
        CREATE INDEX idx_biological_samples_barcode ON biological_samples(sample_barcode);
        CREATE INDEX idx_biological_samples_animal_id ON biological_samples(animal_id);
        CREATE INDEX idx_biological_samples_study_id ON biological_samples(study_id);
        CREATE INDEX idx_biological_samples_protocol_id ON biological_samples(protocol_id);
        CREATE INDEX idx_biological_samples_type ON biological_samples(sample_type);
        CREATE INDEX idx_biological_samples_status ON biological_samples(status);
        CREATE INDEX idx_biological_samples_collection_date ON biological_samples(collection_date);

        -- Create triggers
        CREATE TRIGGER update_biological_samples_timestamp 
          BEFORE UPDATE ON biological_samples 
          FOR EACH ROW 
        ...
        RAISE NOTICE 'Created biological_samples table and supporting structures';
    ELSE
        RAISE NOTICE 'biological_samples table already exists';
    END IF;
END $$;
