...
class ProtocolExtractionService {
   ...
   */
  async extractProtocolData(documentPath, mimeType, documentId) {
    try {
  ...
  async processExtractionJob(jobId) {
    ...
    try {
      ...
      const jobResult = await client.query(
        'SELECT ej.*, pd.file_path, pd.mime_type FROM extraction_jobs ej JOIN protocol_documents pd ON ej.document_id = pd.id WHERE ej.job_id = $1',
        [jobId]
      );
